        ...
        if duplicate_data_id not in file_name:
            ...
            for i_x in range(shape_x):
                ...
                if not os.path.exists(file_out):
                    ...
                    try:
                        sensemap_x = mri.estimate_sense_maps(kspace_x, calib=test_calib, method='jsense', device_id=device_id)
                        sensemap_x = cp.asnumpy(sensemap_x)
                        sensemap_x = np.expand_dims(np.squeeze(sensemap_x), axis=0)
                    except Exception as e:
                        logger.error('  Error in sensitivity calculation for %s\n\tMessage:%s' % (file_out, e))
                    else:
                        # compress the coils
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help='Device id to perform coil sensitivity computation. -1 corresponds to cpu.'
    )
    ...
    if args.num_compressed_coils is None:
        dir_tfrecord = os.path.join(args.output, 'tfrecord_jsense')
    else:
        dir_tfrecord = os.path.join(args.output, 'tfrecord_jsense_croppedmap_cc%d' % args.num_compressed_coils)
    # dir_test_npy = os.path.join(args.output, 'test_npy')
    shape_z, shape_y = setup_data_tfrecords(
        args.dir_npy, dir_tfrecord, device_id=args.device_id)
