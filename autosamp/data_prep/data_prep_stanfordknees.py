...
def ismrmrd_to_np(filename):
    ...
    if logger.getEffectiveLevel() is autosamp.data_prep.utils.logging.logging.DEBUG:
        wrap = tqdm
...
def process_tfrecord(example, num_channels=None, num_maps=None):
    ...
    features = tf.io.parse_single_example(
        ...
        features={
            ...
            'xslice': tf.io.FixedLenFeature([], tf.int64),
            'ks_shape_x': tf.io.FixedLenFeature([], tf.int64),
            ...
            'map_shape_x': tf.io.FixedLenFeature([], tf.int64),
            ...
            'map_shape_m': tf.io.FixedLenFeature([], tf.int64),
    ...
    with tf.name_scope('kspace'):
        ks_record_bytes = tf.io.decode_raw(features['ks'], tf.float32)
